...
E2E는 도메인 단위가 아니라 사용자 여정 단위로 작성한다.  
아래 기준으로 **모두** 해당하는 흐름만 E2E 대상으로 선정한다.

1. **이게 안 되면 사용자가 서비스의 핵심 가치를 얻지 못한다**
2. **거의 모든 사용자가 반드시 거치는 흐름이다**
3. **unit test만으로는 레이어 간 조합 오류를 잡기 어렵다**

판단 시점: 해당 도메인의 `/integrate-domain`이 완료된 후, 팀이 명시적으로 결정한다.

**Critical (E2E 대상)**
- 인증 흐름 (로그인 → 서비스 진입)
- 서비스 핵심 데이터 조회 (목록 → 상세)
- 핵심 데이터 제출 흐름 (폼 작성 → 저장 → 결과 확인)

**Non-critical (unit test로 충분)**
- 필터 / 정렬 / 페이지네이션
- 에러 메시지 문구
- UI 상태 전환 (로딩, 빈 상태)
- 날짜/숫자 포맷 표시

서비스 전체에서 E2E 테스트는 **5~10개** 이내로 유지한다.  
그 이상이 되면 유지 비용이 가치를 초과한다.
